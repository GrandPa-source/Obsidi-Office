...
test('isPendingFork: pending when it identifies its origin and is not reconciled', () => {
...
// The shape saveForkCopy ACTUALLY writes today: no _document.md in the container,
// so forkOf lands as ''. The old rule returned false here, which meant every real
// fork was absent from the reconciliation pane. Fixture taken from the sidecar
// written during the 21e check-out gate run (2026-08-10).
test('isPendingFork: a real fork sidecar with empty forkOf is still pending', () => {
  const real = {
    docx: '[[FanOutPolicy_fork_ojames_20260810.docx]]',
    forkOf: '',
    forkBaseVersion: 'FanOutPolicy_V2.1.docx',
    forkAuthor: 'ojames',
    forkAt: '2026-08-11T03:38:44.422Z',
    forkReason: 'checkout-conflict',
    reconciled: false,
  };
  assert.strictEqual(dc.isPendingFork(real), true);
  assert.strictEqual(dc.isPendingFork({ ...real, reconciled: true }), false);
});

// Guard the negative case: an ordinary sidecar must never read as a fork.
test('isPendingFork: an ordinary document sidecar is not a fork', () => {
  assert.strictEqual(dc.isPendingFork({ title: 'Fan-Out Policy', revision: '2.0' }), false);
  assert.strictEqual(dc.isPendingFork(null), false);
  assert.strictEqual(dc.isPendingFork(undefined), false);
});
...
test('filterNoteFeed: searching also matches noteKind and noteSource', () => {
  const rows = dc.mergeNoteFeed([{ origin: 'A note page', originKind: 'page', originPath: '/a',
    entries: [{ date: '2026-08-01', body: 'note page', noteKind: 'Mention', noteSource: 'Fan-Out Policy' }] }]);
  assert.strictEqual(dc.filterNoteFeed(rows, { text: 'mention' }).length, 1);
  assert.strictEqual(dc.filterNoteFeed(rows, { text: 'fan-out policy' }).length, 1);
  assert.strictEqual(dc.filterNoteFeed(rows, { text: 'nonexistent' }).length, 0);
});
