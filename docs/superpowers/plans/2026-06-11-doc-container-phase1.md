...
- After each view task, **deploy + manual smoke** is the test (Obsidian views can't be unit-tested headlessly). Deploy target: `C:\Obsidian\OB_Testing\.obsidian\plugins\obsidi-office\` (copy `main.js` + `manifest.json` + styles — **not** `lib/`; the core is inlined into `main.js`).
- **iPad is a hard requirement (spec §5.4).** Mobile cannot resolve `require('./lib/...')`, so the pure core is **inlined into `main.js`** via `scripts/inline-doc-container.js` (run it after editing `lib/doc-container.js`); `node --test` still runs against `lib/doc-container.js` directly. Keep `electron`-only calls guarded + degrading on mobile; no top-level `fs`/`path` on mobile. Each rung's iPad smoke gate is mandatory.
...
- [ ] **Step 1: Make the pure core available in main.js — INLINED (mobile-safe), not require'd**

Obsidian mobile can't resolve `require('./lib/...')`. Keep `lib/doc-container.js` as the test source-of-truth, but **inline** it into `main.js` (mirrors the existing `FFLATE_UMD_SOURCE`). Add a marker region near the top of `main.js` (after `const obsidian = require('obsidian');`):
...
Create `scripts/inline-doc-container.js` (wraps the whole module in an IIFE so its own `module.exports` is captured — no need to enumerate exports):
...
Run `node scripts/inline-doc-container.js` after every change to `lib/doc-container.js`. `node --test lib/doc-container.test.js` is unaffected (it requires the module directly).
