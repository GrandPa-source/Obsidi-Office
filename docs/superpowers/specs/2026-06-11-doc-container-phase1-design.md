...
### Carried from the critical review (2026-06-12)

- **(Critique #3) Status must *act* — a review/approval loop.** Status + next-review are currently passive. Add: an **`owner`** field per document; a **"Needs attention"** surface (overdue reviews + pending-approval) at vault/category level — derivable from existing metadata, so it can land early (~v0.4). The full **approval workflow** (reviewer→approver routing, Pending Review/Approval queues, status-transition rules, `statusHistory`, review reminders/notifications) is the deferred **QMS workflow phase** (post-Phase 5). Without this loop the rich status/lifecycle metadata stays decorative.
- **(Critique #4) Global search, saved views, reporting/export.** Per-container search + rollups exist; missing is **vault-wide** querying ("every overdue policy", "every doc invoking PHIPA", "approved as of date"), **saved views**, and **export** — a review calendar, an accreditation **evidence index**. For accreditation the *output* is the actual deliverable. Likely its own phase; can partly ride Dataview. **Corpus synergy:** these cross-taxonomy queries are exactly what the agent answers — design the index once, serve both.
- **(Critique #5) Onboarding existing documents + performance/caching.** The real vault already holds `governance/`/`projects/` files. Need a **scan-and-adopt / backfill** step (create sidecars, assign container types) so the feature works on the existing structure, not only new docs. And the live `vault.getFiles()` + per-sidecar frontmatter reads on every render (rollups, cross-document aggregation) need a **cached index** at scale, invalidated on vault events. Both are "works in the demo, breaks on the real vault" risks — confront them when dogfooding v0.1 on the real folder (the ship-gate).
- **(Critique #6) Schema versioning + sync robustness.** As the sidecar sub-structures (`noteLog`/`activityLog`/`stakeholders`/`relatedDocuments`/`definitions`) + project notes grow, add a **`schemaVersion`** field + a **migration** step (upgrade older sidecars on read). Across desktop↔iPad Sync these YAML blobs are merge-conflict-prone — document a stance: last-write-wins on scalar fields, append-merge for append-only logs where feasible, and treat any derived index (Critique #5) as **rebuildable** so a conflict is never fatal.
